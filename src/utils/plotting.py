...
def plot_log_info(config_info, mode):
    if mode == "segmentation":
        ...
        fig, ax = plt.subplots(2, 1, figsize=(20, 10 * 2))
        sns.lineplot(data=log_file, x="epoch", y="train_loss", ax=ax[0], label="train_loss")
        sns.lineplot(data=log_file, x="epoch", y="val_loss", ax=ax[0], label="val_loss")
        ax[0].set_title("Loss, " + title_suffix)

        sns.lineplot(data=log_file, x="epoch", y="avg_train_f1", ax=ax[1], label="train_f1")
        sns.lineplot(data=log_file, x="epoch", y="avg_val_f1", ax=ax[1], label="val_f1")
        ax[1].set_title("F1, " + title_suffix)
    else:
